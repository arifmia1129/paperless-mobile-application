...
export default function OfflineApplications() {
    const { applications, handleApplications } = useContext(AuthContext);

    const handleDeleteApplication = async (id) => {
        const allApplication = await AsyncStorage.getItem("applications");

        const applications = JSON.parse(allApplication);

        const exists = applications.filter(application => application.id !== id);

        await AsyncStorage.setItem("applications", JSON.stringify(exists));
        handleApplications();
    }
